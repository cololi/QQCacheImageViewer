...
// 并发控制
const SHARP_CONCURRENCY = 4; // 限制sharp并发数
const FILE_BATCH_SIZE = 50; // 每批处理的文件数
  ...
  try {
    ...
    for (let i = 0; i < files.length; i += FILE_BATCH_SIZE) {
      ...
      const results = await Promise.all(
        batch.map(file => processFile(file, basePath, yearMonth, oriPath))
      );
...
export const scanQQCacheDirectory = async (
  ...
  if (basePaths.length === 0) {
    throw new Error('No paths provided to scan');
  }
