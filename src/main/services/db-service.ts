...
export const saveImages = (images: ImageMetadata[]): boolean => {
  if (!db) return false;

  try {
    const stmt = db.prepare(`
      INSERT OR REPLACE INTO images
      (hash, year_month, format, file_path, thumb_path, file_size, width, height, ratio, file_time, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
    `);

    const insertMany = db.transaction((imgs: ImageMetadata[]) => {
...
export const getImages = (params: QueryParams): Image[] => {
  ...
  try {
    ...
    // 基于实际文件时间进行年月筛选
    if (params.yearMonth) {
      query += " AND strftime('%Y-%m', file_time) = ?";
    ...
    return results.map(row => ({
      ...
      height: row.height,
      ratio: row.ratio,
      fileTime: row.file_time ? new Date(row.file_time).toISOString() : undefined,
...
export const getImageCount = (yearMonth?: string): number => {
  ...
  try {
    ...
    if (yearMonth) {
      // 基于实际文件时间进行年月筛选
      query += ` WHERE strftime('%Y-%m', file_time) = ?`;
...
export const getMonths = (): Array<{ yearMonth: string; count: number }> => {
  ...
  try {
    // 基于实际文件时间(file_time)生成年月列表，而不是文件夹路径(year_month)
    const query = `
      SELECT 
        strftime('%Y-%m', file_time) as yearMonth, 
        COUNT(*) as count
      FROM images
      WHERE file_time IS NOT NULL
      GROUP BY strftime('%Y-%m', file_time)
