...
export const queryImages = (filters: {
  ...
  try {
    ...
    // Add sorting (whitelist field, fixed direction — prevents SQL injection)
    query += safeOrderBy(filters.sortField, filters.sortOrder);

    // Always apply pagination — clamped values are guaranteed safe and positive
    query += ' LIMIT ? OFFSET ?';
    sqlParams.push(clampLimit(filters.limit), clampOffset(filters.offset));
...
export const getImageStats = (): {
  ...
  try {
    ...
    const dateRangeQuery = `
      ...
      FROM images
    `;
    const dateResult = db.prepare(dateRangeQuery).get() as {
      minDate: string;
      maxDate: string;
    } | null;
 ...
 */
export const searchImages = (keyword: string, limit: number = 100): Image[] => {
...
export const clearOldCache = (daysOld: number = 90): boolean => {
  ...
  if (!Number.isInteger(daysOld) || daysOld <= 0) {
    console.error('clearOldCache: invalid daysOld', daysOld);
    return false;
  }

  try {
    const stmt = db.prepare(`DELETE FROM images WHERE updated_at < datetime('now', ?)`);
